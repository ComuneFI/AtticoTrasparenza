...
@Component({
  selector: 'app-tabella-del',
  templateUrl: './tabella-del.component.html',
  styleUrls: ['./tabella-del.component.scss']
})
export class TabellaDelComponent implements OnInit, OnDestroy {
  json_list: any[] = [];
  columns: string[] = [];
  columnLabels: string[] = [];
  isLoading: boolean;
  ...
  constructor(private http: HttpClient,
        ...
        this.columnLabels = [
            ...
            this.translateService.instant('columnLabels')['executionDate'],
            this.translateService.instant('columnLabels')['publicationDate'],
            this.translateService.instant('columnLabels')['office'],
            // this.translateService.instant('columnLabels')['speaker'],
            // this.translateService.instant('columnLabels')['competence'],
  ...
  public retrieveData(): number {
    ...
    onSearch(data: any, params?: any) {
        ...
        if (data.competenza === environment.atto.tipo.delibereGiunta.codice) {
          const list: any[] = [];
          for (const item of filters.select.deliberazioni.giunta.tipoAtti) {
              list.push(item.codice);
          }
          postRequestData['tipiAtto'] = list;
        } else if (data.competenza === environment.atto.tipo.delibereConsiglio.codice) {
          const list: any[] = [];
          for (const item of filters.select.deliberazioni.consiglio.tipoAtti) {
              list.push(item.codice);
          }
          postRequestData['tipiAtto'] = list;
        } else {
        ...
        // console.log('TEST SERACH ', postRequestData);
        this.isLoading = true;
        this.http.post(environment.endpoint.post.searchAtti, postRequestData).subscribe(
        pdata => {
            this.json_list = [];
